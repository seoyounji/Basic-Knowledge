...
2개의 stack으로 queue를 구현하려면 일단 데이터를 모두 stack에 넣은 다음 하나씩 빼면서 다시 다른 stack에 넣고 2번째 스택에서 다시 데이터를 하나씩 빼면 된다.
...
* leaf 노드를 제외한 모든 노드의 값이 각각의 왼쪽 서브 트리를 구성하는 모든 노드의 값보다 크다
* leaf 노드를 제외한 모든 노드의 값이 각각의 오른쪽 서브 트리를 구성하는 모든 노드의 값보다 작다
...
Complete Binary Tree이기 때문에 배열을 이용해 관리할 수 있으며 인덱스를 통한 random access 또한 가능하다. 인덱스 번호는 i번째 노드에 대해 왼쪽 자식 노드는 i * 2, 오른쪽 자식 노드는 i * 2+1 이 된다.         

즉 heap은 우선 순위에 따라 데이터를 정렬한 tree 인 것이다.        

최대값 및 최소값을 찾아내는 연산을 빠르게 하기 위해 고안된 구조이며 BST와는 다르게 중복값을 허용한다.     

모든 부모 노드의 값이 각각의 자식 노드의 값보다 크거나 같은 Max Heap(최대 힙)과          

그 반대로 모든 부모 노드의 값이 각각의 자식 노드의 값보다 작거나 같은 Min Heap(최소 힙)이 있다.       

Max Heap에서는 루트 노드에 있는 값이 가장 크고 Min Heap에서는 루트 노드에 있는 값이 가장 작다.        

​                    

* 삽입
  * 일단 힙의 마지막 노드에 삽입한 뒤 부모 노드의 값과 비교해 교환하거나 가만히 둔다.
* 삭제
  * 무조건 루트 노드의  값을 삭제하고 heap을 재정렬한다.

​                

heap을 사용해 데이터를 정렬하는 방법에는 2가지가 있다.       

하나는 데이터들을 heap에 넣었다가 하나씩 꺼내는 것이고 다른 하나는 기존 배열을 heapify (heap으로 만들어주는 과정) 를 거쳐 꺼내는 것이다.

Max heap을 만드는 max-heapify는 루트 노드의 값이 자식 노드보다 작으면 2개의 자식 노드 중 값이 큰 노드와 루트 노드를 교환하는 과정을 더 이상 교환할 노드가 없을 때까지 반복하면 된다.       

마찬가지로 Min heap을 만드는 min-heapify 역시 루트 노드의 값이 자식 노드보다 크면 2개의 자식 노드 중 값이 작은 노드와 루트 노드를 교환하면 된다.        

더 자세한 내용은 정렬 파트에서 다룬다.        
...
이 구조는 내부적으로 배열을 이용해 데이터를 저장한다.        

해시 알고리즘을 이용해 고유한 인덱스를 가지며 이 인덱스를 이용해 빠른 검색을 할 수 있다.        

해시 알고리즘이란 임의의 길이의 데이터를 수학적 연산을 통해 고정된 길이의 데이터로 매핑하는 알고리즘이다. 이 알고리즘에 의해 얻어지는 값을 해시라고 하며 이를 고유한 인덱스로 사용한다.         

​                    

해시 테이블은 해시 값과 데이터 값을 매핑한 데이터 구조이다.        

해시 테이블은 탐색,삽입,삭제의 시간 복잡도가 O(1)일 정도로 적은 자원으로 많은 데이터를 효율적으로 관리하기 쉽다는 장점이 있지만 해시 알고리즘에 의해 해시 값에 중복이 생길 수도 있다는 단점이 있다. 이를 해시 충돌이라 하고 이를 해결하기 위한 2가지 방법이 있다.           

...
해시 알고리즘이란 임의의 길이의 데이터를 수학적 연산을 통해 고정된 길이의 데이터로 매핑하는 알고리즘이다. 왜 하필이면 고정된 길이여야 하면은, 길이가 무한하다면 이 해시 알고리즘을 사용하는 이유가 없기 때문이다.       

이 알고리즘에 의해 얻어지는 값을 해시라고 하며 이를 고유한 인덱스로 사용한다.         

​                    

해시 테이블은 해시 값과 데이터 값을 매핑한 데이터 구조이다.        

<img src="images/hash2.JPG" title="tree" alt="tree"></img>

해시 테이블은 탐색,삽입,삭제의 시간 복잡도가 O(1)일 정도로 적은 자원으로 많은 데이터를 효율적으로 관리하기 쉽다는 장점이 있지만 사실 해시 알고리즘의 출력값의 가짓수가 유한하므로 해시 값이 중복되는 일이 반드시 발생한다는 단점이 있다. 이를 해시 충돌(Hash Collision) 이라 하고 이를 해결하기 위한 2가지 방법이 있다.           

* chaining (closed addressing)
  
  * 각 해시 값에 매핑되는 데이터를 linked list를 이용해 저장하는 방식으로 데이터를 검색할 때 선형 탐색을 하기 때문에 시간 복잡도가 O(N)으로 느리다는 단점은 있지만 linked list 대신 red black tree 구조를 사용하면 O(log N)으로 빨라진다.
  
    <img src="images/hash1.JPG" title="tree" alt="tree"></img>

​          

* open addressing

  * 해시 알고리즘으로 얻은 곳이 아닌 데이터가 아직 저장되지 않은 다른 곳을 찾아 데이터를 저장하는 방식이다.

  * 선형 탐사 (linear probing) : 현재 주소에서 고정 크기만큼 뒤의 주소로 이동해 데이터를 저장

  * 제곱 탐사 (quadratic probing) : 현재 주소에서 제곱수만큼(1,4,9,16...) 뒤의 주소로 이동해 데이터를 저장

  * 이중 해싱 (double hashing) : 다른 해시 알고리즘을 한번 더 적용한 뒤 나온 주소에 데이터를 저장

  * 재해싱 (rehashing) : 해시 테이블의 크기를 늘리고 늘어난 테이블의 크기에 맞춰 모든 데이터를 다시 해싱

  * 이 방법은 데이터 삭제 처리시에 문제가 발생할 수 있는데 아래 그림을 보며 이해해보자.

    <img src="images/hash3.JPG" title="tree" alt="tree"></img>

    위 그림은 모두 해시 테이블의 인덱스 값만 나타낸 것이다. 첫번째 이미지와 같은 상황에서 인덱스 1에 해당하는 데이터를 삭제하면 두번째 이미지와 같은 상황이 된다. 이 때 새로운 데이터를 삽입하는데 이 데이터를 해싱 처리했더니 나온 값이 38이라고 해보자. 해시 테이블의 인덱스를 모두 검사해 해시 충돌이 일어났는지 확인해야하는데 이 검사는 인덱스가 빈 곳이 나타나면 자동으로 멈추고 그 위치에 인덱스를 저장한다. 그럼 동일한 해시 값이 해시 테이블에 저장되는 사태가 일어나는 것이다.         

    이런 문제점을 해결하기 위해선 데이터를 삭제한 뒤 인덱스에 더미값을 집어넣는 것만 하면 된다. 이 더미값은 실제로 값을 가지진 않고 그저 해시 테이블의 인덱스 검사가 끝까지 이어질 수 있게하는 역할만 하는 것이다. 만약 데이터 삭제가 빈번해 이런 더미값이 많아진다면 해시 테이블을 리빌딩하는 것이 좋다.       
